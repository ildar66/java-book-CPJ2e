...
/**
 * This class is interoperable with java.beans.VetoableChangeSupport, but relies on a streamlined copy-on-write scheme
 * similar to that used in CopyOnWriteArrayList. It also adheres to clarified semantics of add, remove, and
 * fireVetoableChange operations.
 * <p>
 * <b>Sample usage.</b>
 * 
 * <pre>
 ...
 * 
 * 	// vetoable listeners:
 * 	protected VetoableChangeMulticaster vetoers = new VetoableChangeMulticaster(this);
 * 
 * 	// Possibly also some ordinary listeners:
 * 	protected PropertyChangeMulticaster listeners = new PropertyChangeMulticaster(this);
 * 
...
public class VetoableChangeMulticaster implements Serializable {
	...
	/**
	 * Add a VetoableChangeListener to the listener list. The listener is registered for all properties. If the listener
	 * is added multiple times, it will receive multiple change notifications upon any fireVetoableChange.
	 * 
	 * @param listener
	...
	/**
	 * Add a PropertyChangeListener to the listener list if it is not already present. The listener is registered for
	 * all properties. The operation maintains Set semantics: If the listener is already registered, the operation has
	 * no effect.
	 * 
	 * @param listener
	...
	/**
	 * Remove an occurrence of a VetoableChangeListener from the listener list. It removes at most one occurrence of the
	 * given listener. If the listener was added multiple times it must be removed mulitple times. This removes a
	 * VetoableChangeListener that was registered for all properties, and has no effect if registered for only one or
	 ...
	 * fireVetoableChange names that specific property. However, if a listener is registered both for all properties and
	 * a specific property, it will receive multiple notifications upon changes to that property.
	 * 
	 * @param propertyName
	...
	/**
	 * Add a VetoableChangeListener for a specific property, if it is not already registered. The listener will be
	...
	/**
	 * Remove a VetoableChangeListener for a specific property. Affects only the given property. If the listener is also
	 * registered for all properties, then it will continue to be registered for them.
	 * 
	 * @param propertyName
	...
	protected void multicast(PropertyChangeEvent evt) throws PropertyVetoException {
		...
		try {
			...
			PropertyChangeEvent revert = new PropertyChangeEvent(evt.getSource(), evt.getPropertyName(),
	...
	/**
	 * Report a vetoable property update to any registered listeners. Notifications are sent serially (although in no
	 * particular order) to the list of listeners, aborting if one throws PropertyVetoException. Upon this exception,
	 * fire a new event reverting this change to all listeners that have already been notified (ignoring any further
	 * vetos), suppress notifications to all other listeners, and then rethrow the PropertyVetoException.
	 * <p>
	 ...
	 *                if a recipient wishes the property change to be rolled back.
	 */
	...
	/**
	 * Check if there are any listeners for a specific property. If propertyName is null, return whether there are any
	 * listeners at all.
	 * 
	 * @param propertyName
	 *            the property name.
	 ...
	 *             <p>
	 *             At serialization time we skip non-serializable listeners and only serialize the serializable
	 *             listeners.
	 * 
	 */
