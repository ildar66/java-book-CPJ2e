...
/**
 * This class is interoperable with java.beans.PropertyChangeSupport, but relies on a streamlined copy-on-write scheme
 * similar to that used in CopyOnWriteArrayList. This leads to much better performance in most event-intensive programs.
 * It also adheres to clarified semantics of add and remove operations.
 * <p>
 * <b>Sample usage.</b>
 * 
 * <pre>
...
public class PropertyChangeMulticaster implements Serializable {
	 ...
	 * is added multiple times, it will receive multiple change notifications upon any firePropertyChange
	 * 
	 * @param listener
	...
	/**
	 * Remove a PropertyChangeListener from the listener list. It removes at most one occurrence of the given listener.
	 * If the listener was added multiple times it must be removed mulitple times. This removes a PropertyChangeListener
	 * that was registered for all properties, and has no effect if registered for only one or more specified
	 * properties.
	 * 
	 * @param listener
	 ...
	 * firePropertyChange names that specific property. However, if a listener is registered both for all properties and
	...
	/**
	 * Add a PropertyChangeListener for a specific property, if it is not already registered. The listener will be
	...
	/**
	 * Remove a PropertyChangeListener for a specific property. Affects only the given property. If the listener is also
	...
	/**
	 * Report an int bound property update to any registered listeners. No event is fired if old and new are equal and
	 * non-null.
	 * <p>
	...
	/**
	 * Fire an existing PropertyChangeEvent to any registered listeners. No event is fired if the given event's old and
