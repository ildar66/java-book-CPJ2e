...
class ColoredThing {
    ...
    // vetoable listeners:
    protected final VetoableChangeMulticaster vetoers = new VetoableChangeMulticaster(this);

    // also some ordinary listeners:
    protected final PropertyChangeMulticaster listeners = new PropertyChangeMulticaster(this);
    ...
    public void setColor(Color newColor) throws PropertyVetoException {
        ...
        synchronized (this) {
            ...
            if (changePending) { // allow only one transaction at a time
                throw new PropertyVetoException("Concurrent modification", null);
            } else if (newColor == null) { // Argument screening
                throw new PropertyVetoException("Cannot change color to Null", null);
            } else {
                changePending = true;
                oldColor = myColor;
            }
