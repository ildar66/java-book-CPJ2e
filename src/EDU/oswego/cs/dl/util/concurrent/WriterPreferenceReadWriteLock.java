...
public class WriterPreferenceReadWriteLock implements ReadWriteLock {
	...
	/*
	 * A bunch of small synchronized methods are needed to allow communication from the Lock objects back to this
	 * object, that serves as controller
	 */
	...
	/*
	 * Each of these variants is needed to maintain atomicity of wait counts during wait loops. They could be made
	 * faster by manually inlining each other. We hope that compilers do this for us though.
	 */
	 ...
	 * not know whether the wait sets need notification since they don't know preference rules. So, each supports a
	 * method that can be selected by main controlling object to perform the notifications. This base class simplifies
	 * mechanics.
	 **/
